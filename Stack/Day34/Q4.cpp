/*
    -----Design Stack for middle element------
    - Deleting middle element in O(1) time.
    - Inserting middle elemet in O(1) time.
    - Accessing middle element in O(1) time.

    Approach : 
        - Use array (insertion and deletion both will take O(n) time)
        - Singly LinkedList (O(1) : insertion  but moving pointer backwar is not possible after deletion)
      so,
        - we will use Doubly LinkedList : Insertion and deletion will give  
*/
